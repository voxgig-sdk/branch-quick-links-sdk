# BranchQuickLinks SDK test runner
...
class BranchQuickLinksTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            BranchQuickLinksTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = BranchQuickLinksTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = BranchQuickLinksTestRunner.getenv("BRANCH_QUICK_LINKS_TEST_LIVE")
        override = BranchQuickLinksTestRunner.getenv("BRANCH_QUICK_LINKS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = BranchQuickLinksTestRunner.getenv(key)
        ...
        explain = BranchQuickLinksTestRunner.getenv("BRANCH_QUICK_LINKS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["BRANCH_QUICK_LINKS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if BranchQuickLinksTestRunner._test_control is not None:
            return BranchQuickLinksTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                BranchQuickLinksTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            BranchQuickLinksTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = BranchQuickLinksTestRunner.load_test_control()
...
def load_env_local():
    BranchQuickLinksTestRunner.load_env_local()


def env_override(m):
    return BranchQuickLinksTestRunner.env_override(m)


def entity_data(v):
    return BranchQuickLinksTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return BranchQuickLinksTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return BranchQuickLinksTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return BranchQuickLinksTestRunner.load_test_control()


def live_client_options():
    return BranchQuickLinksTestRunner.live_client_options()


def live_delay_ms():
    return BranchQuickLinksTestRunner.live_delay_ms()
