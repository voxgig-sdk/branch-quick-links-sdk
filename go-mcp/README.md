# branch-quick-links-mcp

[MCP](https://modelcontextprotocol.io) server exposing the BranchQuickLinks SDK as
two agent tools — `branch-quick-links_list` and `branch-quick-links_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/branch-quick-links-mcp)
make build

# 2. Provide credentials via the environment
export BRANCH_QUICK_LINKS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user branch-quick-links \
  -- /absolute/path/to/branch-quick-links-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./branch-quick-links-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// branch-quick-links_list: first page of records
{ "entity": "bulk" }
{ "entity": "bulk", "query": { } }

// branch-quick-links_load: one record by id
{ "entity": "bulk", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/branch-quick-links-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user branch-quick-links \
     -- "$PWD"/dist/*/branch-quick-links-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `branch-quick-links_list` and `branch-quick-links_load` tools now appear
   in new sessions. Ask the agent to *"list bulk using branch-quick-links"*
   and it calls `branch-quick-links_list` with `{"entity":"bulk"}`.
...
```sh
export BRANCH_QUICK_LINKS_APIKEY=sk_live_xxx            # API key
export BRANCH_QUICK_LINKS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `branch-quick-links_list` tool
...
### Call the `branch-quick-links_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `branch-quick-links_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `branch-quick-links_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BRANCH_QUICK_LINKS_APIKEY` | API key sent with every request. |
| `BRANCH_QUICK_LINKS_BASE` | Optional override of the API base URL. |
...
```sh
./branch-quick-links-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"branch-quick-links_load","arguments":{"entity":"bulk","query":{"id":1}}}}'
```
