...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("BRANCH_QUICK_LINKS_TEST_LIVE") == "TRUE" ||
		os.Getenv("BRANCH_QUICK_LINKS_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("BRANCH_QUICK_LINKS_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client        *sdk.BranchQuickLinksSDK
	data          map[string]any
