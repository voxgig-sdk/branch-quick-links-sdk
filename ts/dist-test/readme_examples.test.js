...
const __1 = require("..");
const SDK_NAME = 'BranchQuickLinksSDK';
...
const TEST_SEED = { "entity": { "bulk": { "test01": { "id": "test01" } }, "url": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+BranchQuickLinksSDK\b/.test(code) ||
        /\bBranchQuickLinksSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+BranchQuickLinksSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/BranchQuickLinksSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
