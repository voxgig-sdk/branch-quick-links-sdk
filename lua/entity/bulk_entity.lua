...
local BulkEntity = {}
BulkEntity.__index = BulkEntity


function BulkEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BulkEntity)
  self._name = "bulk"
  self._client = client
...
function BulkEntity:get_name()
  return self._name
end


function BulkEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BulkEntity.new(self._client, opts)
end
...
function BulkEntity:mark_deleted()
  self._deleted = true
end


function BulkEntity:deleted()
  return true == self._deleted
end


function BulkEntity:data_set(args)
  if args ~= nil then
...
function BulkEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BulkEntity:match_set(args)
  if args ~= nil then
...
function BulkEntity:stream(action, args, callopts)
...
---@param reqdata BulkCreateData
---@param ctrl? table
---@return Bulk
---@return string? err
function BulkEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function BulkEntity:_run_op(ctx, post_done)
  local utility = self._utility
