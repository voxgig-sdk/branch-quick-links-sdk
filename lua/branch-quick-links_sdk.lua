...
require("branch-quick-links_types")
...
local BranchQuickLinksSDK = {}
BranchQuickLinksSDK.__index = BranchQuickLinksSDK
...
BranchQuickLinksSDK._make_feature = _make_feature


function BranchQuickLinksSDK.new(options)
  local self = setmetatable({}, BranchQuickLinksSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function BranchQuickLinksSDK:options_map()
...
function BranchQuickLinksSDK:get_utility()
  return Utility.copy(self._utility)
end


function BranchQuickLinksSDK:get_root_ctx()
  return self._rootctx
end


function BranchQuickLinksSDK:prepare(fetchargs)
...
function BranchQuickLinksSDK:direct(fetchargs)
...
function BranchQuickLinksSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function BranchQuickLinksSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "BranchQuickLinksSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function BranchQuickLinksSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function BranchQuickLinksSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "BranchQuickLinksSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function BranchQuickLinksSDK:Bulk(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function BranchQuickLinksSDK:Url(data)
...
function BranchQuickLinksSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = BranchQuickLinksSDK.new(sdkopts)
  sdk.mode = "test"
