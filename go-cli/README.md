# branch-quick-links-cli

boru-driven command-line client **and** interactive REPL for the BranchQuickLinks
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/branch-quick-links-cli)
make build

# 2. See usage (words, entities, env vars)
./branch-quick-links-cli --help
...
BRANCH_QUICK_LINKS_BASE=https://api.example.com ./branch-quick-links-cli --help

# 6. No arguments -> interactive REPL
./branch-quick-links-cli
branch-quick-links> /help
branch-quick-links> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/branch-quick-links-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/branch-quick-links-cli --help
   ```
...
Run with no arguments to open a REPL (prompt `branch-quick-links>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `bulk`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/branch-quick-links-cli`. |
...
(`./branch-quick-links-cli <expr>`) and interactively (the REPL), and expressions compose the
