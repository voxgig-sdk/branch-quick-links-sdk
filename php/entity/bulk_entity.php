...
// BranchQuickLinks SDK Bulk entity
...
class BulkEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new BulkEntity($this->_client, $opts);
    }

    /**
     * @param Bulk|array $args Bulk data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = BranchQuickLinksHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Bulk|array The current Bulk data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Bulk fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = BranchQuickLinksHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Bulk fields).
     */
     ...
     * Create a new Bulk.
     *
     * @param BulkCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed BulkCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Bulk|array The created Bulk as an assoc-array at the
     *   SDK boundary; throws BranchQuickLinksError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = BranchQuickLinksHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
