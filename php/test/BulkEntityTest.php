...
class BulkEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set BRANCH_QUICK_LINKS_TEST_BULK_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $bulk_ref01_data = Helpers::to_map(Vs::getprop(
            Vs::getpath($setup["data"], "new.bulk"), "bulk_ref01"));
        $bulk_ref01_data["branch_key"] = $setup["idmap"]["branch_key01"];

        $bulk_ref01_data_result = $bulk_ref01_ent->create($bulk_ref01_data, null);
        $bulk_ref01_data = Helpers::to_map(is_object($bulk_ref01_data_result) && method_exists($bulk_ref01_data_result, 'data_get') ? $bulk_ref01_data_result->data_get() : $bulk_ref01_data_result);
        $this->assertNotNull($bulk_ref01_data);
        $this->assertNotNull($bulk_ref01_data["id"]);
...
function bulk_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["bulk01", "bulk02", "bulk03", "branch_key01"] as $k) {
    ...
    $entid_env_raw = getenv("BRANCH_QUICK_LINKS_TEST_BULK_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["BRANCH_QUICK_LINKS_TEST_BULK_ENTID"]);
