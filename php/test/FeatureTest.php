...
// BranchQuickLinks SDK feature test
//
...
class FtCtrl extends BranchQuickLinksControl
{
...
class FtClient
{
    ...
    private BranchQuickLinksUtility $utility;

    public function __construct(BranchQuickLinksUtility $utility, array $options)
    {
    ...
    public function get_utility(): BranchQuickLinksUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BranchQuickLinksContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new BranchQuickLinksUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (BranchQuickLinksContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BranchQuickLinksConfig::make_feature($name);
    ...
    private static function buildUrl(BranchQuickLinksSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BranchQuickLinksError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BranchQuickLinksResponse($fetched);
        }
    ...
    private function fail(BranchQuickLinksContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BranchQuickLinksContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
