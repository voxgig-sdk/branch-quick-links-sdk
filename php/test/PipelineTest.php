...
// BranchQuickLinks SDK pipeline test
//
...
{
    private static function utility(): BranchQuickLinksUtility
    {
        return new BranchQuickLinksUtility();
    }
    ...
    private static function ctx(array $over = []): BranchQuickLinksContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new BranchQuickLinksContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new BranchQuickLinksOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof BranchQuickLinksError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new BranchQuickLinksOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = BranchQuickLinksMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new BranchQuickLinksOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = BranchQuickLinksMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new BranchQuickLinksOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = BranchQuickLinksMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BranchQuickLinksSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = BranchQuickLinksMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new BranchQuickLinksResponse([]),
            'result' => new BranchQuickLinksResult([]),
        ]);
        [, $err] = BranchQuickLinksMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new BranchQuickLinksSpec([]),
            'response' => null,
            'result' => new BranchQuickLinksResult([]),
        ]);
        [, $err] = BranchQuickLinksMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new BranchQuickLinksSpec(['step' => 's']),
            'response' => new BranchQuickLinksResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new BranchQuickLinksResult([]),
        ]);
        [, $err] = BranchQuickLinksMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new BranchQuickLinksSpec(['step' => 's']),
            'response' => new BranchQuickLinksResponse(self::resp(200, ['v' => 1])),
            'result' => new BranchQuickLinksResult([]),
        ]);
        [, $err] = BranchQuickLinksMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new BranchQuickLinksSpec(['step' => 's']),
            'response' => new BranchQuickLinksResponse(self::resp(200, ['v' => 2])),
            'result' => new BranchQuickLinksResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        BranchQuickLinksMakeResponse::call($ctx);
    ...
    {
        $preset = new BranchQuickLinksResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new BranchQuickLinksSpec([]),
            'response' => new BranchQuickLinksResponse([]),
            'result' => new BranchQuickLinksResult([]),
        ...
        [$got, $err] = BranchQuickLinksMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new BranchQuickLinksResult([])]);
        [, $err] = BranchQuickLinksMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new BranchQuickLinksSpec([]), 'result' => null]);
        [, $err] = BranchQuickLinksMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BranchQuickLinksSpec(['step' => 's']),
            'result' => new BranchQuickLinksResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new BranchQuickLinksOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BranchQuickLinksMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BranchQuickLinksSpec(['step' => 's']),
            'result' => new BranchQuickLinksResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new BranchQuickLinksOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BranchQuickLinksMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BranchQuickLinksResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new BranchQuickLinksSpec([]),
            'result' => new BranchQuickLinksResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = BranchQuickLinksMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = BranchQuickLinksMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new BranchQuickLinksError('boom', 'boom');
        $utility->fetcher = function (BranchQuickLinksContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BranchQuickLinksSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = BranchQuickLinksMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (BranchQuickLinksContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = BranchQuickLinksMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(BranchQuickLinksResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        BranchQuickLinksMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (BranchQuickLinksContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BranchQuickLinksSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = BranchQuickLinksMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new BranchQuickLinksResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new BranchQuickLinksSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = BranchQuickLinksMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = BranchQuickLinksMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = BranchQuickLinksMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new BranchQuickLinksResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, BranchQuickLinksDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new BranchQuickLinksResult(['ok' => false])]);
        $this->expectException(BranchQuickLinksError::class);
        BranchQuickLinksDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new BranchQuickLinksResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', BranchQuickLinksMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        BranchQuickLinksMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new BranchQuickLinksBaseFeature();
        $b = new BranchQuickLinksBaseFeature();
        BranchQuickLinksFeatureAdd::call($ctx, $a);
        BranchQuickLinksFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): BranchQuickLinksBaseFeature
    {
        $f = new BranchQuickLinksBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        BranchQuickLinksFeatureAdd::call($ctx, self::named_feature('a'));
        BranchQuickLinksFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        BranchQuickLinksFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        BranchQuickLinksFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        BranchQuickLinksFeatureAdd::call($ctx, $replace);
        ...
        BranchQuickLinksFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): BranchQuickLinksContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new BranchQuickLinksSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = BranchQuickLinksPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        BranchQuickLinksPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new BranchQuickLinksResponse(['headers' => null]),
            'result' => new BranchQuickLinksResult([]),
        ]);
        BranchQuickLinksResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        BranchQuickLinksResultBody::call($ctx);
