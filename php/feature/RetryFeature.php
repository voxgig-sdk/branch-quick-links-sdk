...
// BranchQuickLinks SDK retry feature
...
class BranchQuickLinksRetryFeature extends BranchQuickLinksBaseFeature
{
    ...
    public function init(BranchQuickLinksContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (BranchQuickLinksContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(BranchQuickLinksContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(BranchQuickLinksContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
