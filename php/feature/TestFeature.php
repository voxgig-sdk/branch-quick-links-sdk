...
// BranchQuickLinks SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class BranchQuickLinksTestFeature extends BranchQuickLinksBaseFeature
{
    ...
    public function init(BranchQuickLinksContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (BranchQuickLinksContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = BranchQuickLinksParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (BranchQuickLinksContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(BranchQuickLinksContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = BranchQuickLinksParam::call($ctx, $k);
