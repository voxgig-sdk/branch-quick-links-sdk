...
//    only - a typed BranchQuickLinksContext there would make every ctx
...
final class BranchQuickLinksOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new BranchQuickLinksContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new BranchQuickLinksSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new BranchQuickLinksResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new BranchQuickLinksError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new BranchQuickLinksResponse($clean['response']);
    ...
    public static function err_from_map($m): ?BranchQuickLinksError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new BranchQuickLinksError(is_string($code) ? $code : '', $msg);
    }
