...
class BranchQuickLinksMakeError
{
    public static function call(?BranchQuickLinksContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new BranchQuickLinksContext([], null);
        }
        $op = $ctx->op ?? new BranchQuickLinksOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new BranchQuickLinksResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof BranchQuickLinksError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new BranchQuickLinksError('', $msg, $ctx);
        ...
        if ($err instanceof BranchQuickLinksError) {
